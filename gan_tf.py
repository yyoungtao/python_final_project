...
for it in range(53000):
    ...
    targets_D.append(D_loss_curr)
    targets_G.append(G_loss_curr)
    if it % 100 == 0:
        print('Iter: {}'.format(it))
        print('D loss: {:.4}'. format(D_loss_curr))
        print('G_loss: {:.4}'.format(G_loss_curr))
print()
plt.plot(range(0, 50000), targets_D[3000:])
plt.yscale('log')
plt.show()
plt.plot(range(0, 50000), targets_G[3000:])
plt.show()

#test the discriminator

#Xt_mb, Yt_mb = mnist.train.next_batch(mb_size)

#Dt_real, Dt_logit_real = dc_discriminator(X)
#Dt_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_real, labels=Yt_mb))
#D_solver = tf.train.AdamOptimizer().minimize(D_loss, var_list=theta_DC_D)